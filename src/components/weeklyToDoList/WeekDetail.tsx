    ...
    <Card className="bg-bgLightBox dark:bg-bgDarkBox w-full p-3">
      <CardHeader>
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <Dialog open={isEditDialogOpen} onOpenChange={setEditDialogOpen}>
              <DialogTrigger>
                <Button className="w-[40px]" variant={'default'}>
                  <FaRegEdit />
                </Button>
              </DialogTrigger>
              ...
              <DialogContent>
                <DialogHeader>
                  <DialogTitle>Change Weekly To-do List Information</DialogTitle>
                </DialogHeader>
                
                <EditWeek title={title} desc={desc} date={date} onClose={() => setEditDialogOpen(false)} />
              </DialogContent>
            </Dialog>

            <Dialog open={isDeleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
              <DialogTrigger>
                <Button className="w-[40px]" variant={'destructive'}>
                  <RiDeleteBin6Line />
                </Button>
              </DialogTrigger>
              ...
              <DialogContent>
                ...
                <DeleteWeek title={title} onClose={() => setDeleteDialogOpen(false)} />
              </DialogContent>
            </Dialog>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between text-gray-800 dark:text-gray-100">
          <p className="text-xl text-gray-500 dark:text-gray-400 font-semibold">{getWeekRange(date)}</p>
